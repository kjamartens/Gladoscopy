...
[] **The napari-plugin entry path (`_dock_widget.MainWidget`, *Plugins → Run Glados-PycroManager*) looks unrunnable end-to-end** (found 2026-09-21 during the layout-toolkit work; not run, read only). `MainWidget` sets `shared_data.core` but never a `MILcore`, yet `MDAGlados` and `ConfigInfo` go through `shared_data.MILcore` for everything; `microManagerControlsUI_plugin` also calls the Java-only `get_available_config_groups().size()`. Its `ConfigInfo(core, id)` arity bug is fixed and `GladosWidget` now drives toolkit panels through their `ResponsiveGrid`, but nobody has launched this path in a long while. Decide whether to repair it (build a MIL the way `GUI_napari.main` does) or retire it in favour of the standalone `glados` entry.
[] **`pSMLM.py`'s `visualise()` has the same napari async-slicing defect that was just fixed in `pSMLM_live.py`** (reported 2026-09-17 as `IndexError: index 52 is out of bounds for axis 0 with size 52` out of `Points._view_data` / `_update_slice_response`). It assigns `napariLayer.data = coords` and *then* sets `selected_data`, `symbol`, `size`, `face_color`, `border_color` and `border_width` — on every frame. Each of those emits a napari event whose handler re-reads the layer's view data through the indices of the last *completed* slice, and napari 0.7 slices asynchronously, so right after the point count changes those indices are stale. In `pSMLM.py` most of the style assignments are individually wrapped in `try/except Exception`, which swallows the symptom but not the `_on_slice_ready` variant that fires on napari's own thread. Fix is the same shape as `pSMLM_live._style_points`: apply the style once, clear a non-empty selection *before* the data shrinks, and assign `.data` last. Left untouched here only to keep existing nodes' behaviour unchanged — worth doing, since this is the node in day-to-day use.
[] **Follow-up to the RT multi-layer/replay work (implemented 2026-09-17, needs a hardware check).** The two `[~]` items below are implemented and unit-tested, but never run against a microscope or through the full GUI worker. To verify: run an MDA with `vis_method='multiDstack'` and the new `pSMLM live + SR` node, confirm the three layers appear (analysed frame + localizations overlaid, SR panel beside them, grid mode **off**), then drag the time slider after the acquisition and confirm the localizations follow it. Scrub to a frame the drop-gate skipped and confirm the log shows an on-demand re-analysis, then scrub back and confirm the revisit is instant.
[] **"Analyse every MDA frame" was requested as lossless, and is not implemented.** It cannot be delivered as specified: true losslessness needs backpressure onto the acquisition, and on `MMCORE_PLUS` the upstream `FrameRing` is overwrite-oldest (capacity 256), so frames would be shed before the RT queue ever saw them. What *is* deliverable is a best-effort deep queue (raise the node deque from `maxlen=10` and replace the `len(queue) < 1` gate with `len(queue) < maxlen` while `mdaMode`, logging the per-node drop tally). Meanwhile the on-demand re-analysis on a history miss already gives the user-visible outcome. Decide whether to build the deep queue under an honest name.
[] **`MMcontrols.enableAutoSliceAnalysis` is still a parallel `dims.events.current_step` subscriber** (`MMcontrols.py:1729`), undebounced and running the analysis on the GUI thread. It predates `RTReplayController` and does the same job worse. `shared_data.rt_replay.suspended` exists so the two can coexist but nothing sets it, so with both active a scrub triggers two independent re-renders. Refactor the auto-slice button onto the controller.
[] **`pSMLM_image` declares no `__snapshot_attrs__` while its `visualise()` reads `self.sr_canvas` and `self.fullSMLMlocs`.** Since RT nodes are subprocess-isolated by default (T-G10), its overlay is already silently frozen whenever isolation applies — a pre-existing bug, found while adding `__replayable__` and deliberately left alone to keep the feature commits clean. Note it cannot simply declare them: it accumulates *inside* `visualise()`, which is also why it is `"__replayable__": False`. `pSMLM_live.py` shows the shape a fixed version would take.
[] **`pSMLM_image` hardcodes `napariLayer.scale = (0.1, 0.1)`**, ignoring `self.pxsizeum`, so its SR layer's world extent is wrong by the pixel size for any real objective. Pre-existing; found alongside the above.
[] **Nodz-named MDA layers never reach `finalisationProcedure=True`.** `napariGlados.py:2035` gates on `layerName == 'MDA'` *and* reads `shared_data.config.mda_config.backend_method`, where every other branch in the file reads `vis_method`. So a Nodz-driven acquisition (which names the layer after its node) skips the backfill/refresh pass entirely. Looks like a pre-existing bug rather than intent; confirm before relying on that flag as "acquisition over".
