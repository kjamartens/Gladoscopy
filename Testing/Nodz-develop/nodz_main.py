...
# if QtCore.IsPyQt5 or QtCore.IsPyQt4:
...
class Nodz(QtWidgets.QGraphicsView):
    ...
    signal_UndoRedoModifySelection = QtCore.pyqtSignal(signalObject, signalObject, signalObject) # node id list before, node id list after. signal_NodeSelected does not send previous selection
    signal_UndoRedoDeleteSelectedNodes = QtCore.pyqtSignal(signalObject, signalObject) # list of deleted nodes (user data copies). signal_NodeDeleted does only send deleted node names, too late to get their userData for redo
    # # signal_UndoRedoEditNodeName = QtCore.pyqtSignal(signalObject, str, str) # node name before, node name after UNUSED
    signal_UndoRedoAddNode = QtCore.pyqtSignal(signalObject, signalObject) # node added user data. For consistency with signal_UndoRedoDeleteSelectedNodes (we may actually store undo via signal_NodeCreated, but would be called a lot of time from loadGraph)
    signal_UndoRedoMoveNodes = QtCore.pyqtSignal(signalObject, signalObject, signalObject, signalObject) # node name list, fromPos list, toPos list. signal_NodeMoved does not send previous position
    signal_UndoRedoConnectNodes = QtCore.pyqtSignal(signalObject, signalObject, signalObject)  # list of removed ConnectionInfo (potentially due to addition), list of new ConnectionInfo. Could deal with it with plug/socket connected / disconnected but would be tedious with a lot of calls

    signal_dropOnNode = QtCore.pyqtSignal(signalObject, signalObject) #nodzInst, nodeItem

    signal_StartCompoundInteraction = QtCore.pyqtSignal(signalObject) # starts user interaction on a nodz
    signal_EndCompoundInteraction = QtCore.pyqtSignal(signalObject, bool) # end user interaction on a nodz

    signal_SceneDoubleClicked = QtCore.pyqtSignal()

    signal_NodeCreated = QtCore.pyqtSignal(signalObject)
    signal_NodePreDeleted = QtCore.pyqtSignal(signalObject)
    signal_NodeDeleted = QtCore.pyqtSignal(signalObject)
    signal_NodeEdited = QtCore.pyqtSignal(signalObject, signalObject)
    signal_NodeSelected = QtCore.pyqtSignal(signalObject)
    signal_NodeMoved = QtCore.pyqtSignal(signalObject, signalObject)
    # signal_NodeRightClicked = QtCore.pyqtSignal(signalObject)
    signal_NodeDoubleClicked = QtCore.pyqtSignal(signalObject)

    signal_ViewContextMenuEvent = QtCore.pyqtSignal(signalObject) # view context menu event
    signal_NodeContextMenuEvent = QtCore.pyqtSignal(signalObject, signalObject) # node context menu event, node name

    signal_AttrCreated = QtCore.pyqtSignal(signalObject, signalObject)
    signal_AttrDeleted = QtCore.pyqtSignal(signalObject, signalObject)
    signal_AttrEdited = QtCore.pyqtSignal(signalObject, signalObject, signalObject)

    signal_PlugConnected = QtCore.pyqtSignal(signalObject, signalObject, signalObject, signalObject)
    signal_PlugDisconnected = QtCore.pyqtSignal(signalObject, signalObject, signalObject, signalObject)
    signal_SocketConnected = QtCore.pyqtSignal(signalObject, signalObject, signalObject, signalObject)
    signal_SocketDisconnected = QtCore.pyqtSignal(signalObject, signalObject, signalObject, signalObject)
    ...
    signal_KeyPressed = QtCore.pyqtSignal(signalObject)
    signal_Dropped = QtCore.pyqtSignal()

    signal_dragEvent = QtCore.pyqtSignal(signalObject, signalObject) # dragDropEvent, nodzInst
    signal_dragMoveEvent = QtCore.pyqtSignal(signalObject, signalObject) # dragDropEvent, nodzInst
    signal_dropEvent = QtCore.pyqtSignal(signalObject, signalObject) # dragDropEvent, nodzInst
