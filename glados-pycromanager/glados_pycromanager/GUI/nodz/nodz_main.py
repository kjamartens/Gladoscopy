...
class Nodz(QtWidgets.QGraphicsView):
    ...
    def loadGraph_KM(self, filePath='path'):
        ...
        for name in nodesName:
            ...
            node.setX(nodesData[name]['position'][0]-node.baseWidth/2) #type:ignore
            node.setY(nodesData[name]['position'][1]-node.height/2) #type:ignore
        ...
        if 'SCANNING_WIDGET_INFO' in data:
            self.scanningWidget.currentMode = data['SCANNING_WIDGET_INFO']['currentMode']
            self.scanningWidget.scanMode = data['SCANNING_WIDGET_INFO']['scanMode']
            for scanModeID in range(len(self.scanningWidget.scanArray_modes)):
                scanMode = self.scanningWidget.scanArray_modes[scanModeID][0]
...
class NodeScene(QtWidgets.QGraphicsScene):
    ...
    def drawBoundingBoxes(self,painter):
        ...
        if len(NodeListConnectionsAcq) > 0:
            self.drawBoundingBox(painter,NodeListConnectionsAcq,boundingBorder=35,solidPaint=[150,30,150,75],linePaint=[180, 36, 180, 200])
