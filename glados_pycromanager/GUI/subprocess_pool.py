...
def _blank_worker_bootstrap(assign_queue, ready_queue):
    ...
    (rt_analysis_info, in_queue, out_queue, stop_event,
     control_in_queue, control_out_queue, log_level) = assignment
...
class WarmSubprocessPool:
    ...
    def try_claim(self):
        """Returns (process, assign_queue) if a warm process is ready right
        now, else None (never blocks -- this is called from the Qt GUI
        thread). On success, the caller owns the process and must
        assign_queue.put(...) a real assignment; a replacement blank process
        is started in the background immediately."""
        ...
        try:
            ...
            return None
        claimed = (self._process, self._assign_queue)
        self._process = None
        self._assign_queue = None
        self._ready_queue = None
