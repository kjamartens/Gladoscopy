...
#deprecated alias for the dev-scratch Main.py until ARCH-7 removes it.
...
def createFunctionWithArgs(functionname,*args):
    ...
    import warnings
    warnings.warn(
        "HelperFunctions.createFunctionWithArgs is a display-only helper now; "
...
def createFunctionWithKwargs(functionname,**kwargs):
    ...
    import warnings
    warnings.warn(
        "HelperFunctions.createFunctionWithKwargs is a display-only helper now; "
        "eval'ing its result is unsafe — use "
        "glados_pycromanager.autonomous.registry.dispatch_from_eval_text instead.",
        DeprecationWarning,
        stacklevel=2,
