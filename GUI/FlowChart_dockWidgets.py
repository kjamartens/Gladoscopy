...
class flowChart_dockWidgetF(nodz_main.Nodz):
    def __init__(self,core=None,shared_data=None,MM_JSON=None):
        ...
        # Node A
        nodeA = self.createNode(name='nodeA', preset='node_preset_1', position=QtCore.QPoint(5010,5010))

        self.createAttribute(node=nodeA, name='Aattr1', index=-1, preset='attr_preset_1',
                            plug=True, socket=False, dataType=int)

        self.createAttribute(node=nodeA, name='Aattr2', index=-1, preset='attr_preset_1',
                            plug=False, socket=True, dataType=int)

        # # Node B
        nodeB = self.createNode(name='nodeB', preset='node_imaging',position=QtCore.QPoint(5070,5010))

        self.createAttribute(node=nodeB, name='Battr1', index=-1, preset='attr_preset_1',
                            plug=True, socket=False, dataType=int)

        self.createAttribute(node=nodeB, name='Battr2', index=-1, preset='attr_preset_1',
                            plug=True, socket=False, dataType=int)
    ...
    def contextMenuEvent(self, QMouseevent):
        ...
        new_acq_node_action = QAction("New2 Acquisition node", self)
        new_analysis_node_action = QAction("New2 Analysis node", self)
        new_scoring_node_action = QAction("New2 Scoring node", self)
        new_decision_node_action = QAction("New2 Decision node", self)
