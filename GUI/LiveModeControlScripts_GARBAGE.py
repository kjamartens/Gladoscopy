...
class ThreadOverlay(QThread):
    #Testing of a live overlay. Not sure what what but eh
    def TestLiveOverlay(self,layer_name):
        lock.acquire()
    ...
    def continuous_task(self):
        ...
        while not stop_continuous_task:
            ...
            self.TestLiveOverlay('live_overlay')
            total_th1+=1
            # time.sleep(1)
    
    def continuous_task_start(self):
...
def napariUpdateLive(liveImage):
    ...
    continuous_thread = threading.Thread(target=ThreadOverlay.continuous_task_start(thread))
